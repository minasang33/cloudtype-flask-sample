...
# token
# https://api.telegram.org/bot6056998714:AAHa9RfCHGMf68cqD9hNSvO1VkQS0gp6lhY/getUpdates
...
def getJS(interval):
    ...
    for i in USDT_ticker:
        ...
        ticker = arr['ticker'].split("/")
        rsi = str(arr['RSI']).split(".")
        printValue= str(ticker[0])+"("+ str(rsi[0])+")"
        #종가가 상한선 이상일때 & RSI값이 70이상일때
        if arr['Close'] > arr['ubb'] and arr['RSI'] >= 70:
            result_u.append(printValue)
        if arr['Close'] < arr['lbb'] and arr['RSI'] <= 30:
            result_l.append(printValue)
...
def send_links():
    ...
    sendMessage(bot, '15분봉 \n 상: '+str(fifteen_ticker[0])+'\n 하: '+str(fifteen_ticker[1]))
